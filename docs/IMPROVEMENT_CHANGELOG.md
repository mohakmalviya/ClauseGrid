...
| MiMo hardening (unscored) | Repeated the M10 manager/falsifier run through CommandCode MiMo V2.5 and fixed each observed runtime failure: premature token/tool exhaustion, oversized parallel observations, empty terminal streams, lost citation handles after compaction, and ignored mandatory tool choice. | Run `agent-702385dc5cee-771cae24` completed `REPAIR` after 8 manager turns, 14 falsifier turns, 36 tool calls, and 7 sandbox executions. Five candidate-bound falsifier experiments passed and the 120-event trajectory verifies. | Kept the provider-neutral fixes. This is one end-to-end compatibility run, not a repeated benchmark or evidence that MiMo is the production model winner. |
