...
const EXCEL_FORMULA_CALL=/^=(?:IF|AND|OR|MAX|MIN|ROUND|LOOKUP|COUNTIF)\s*\(/i;
function splitFormulaSegments(value){const source=String(value||'').trim(),segments=[];let cursor=0;for(let i=0;i<source.length;i++){if(source[i]!=='=')continue;const opener=source.slice(i).match(EXCEL_FORMULA_CALL);if(!opener)continue;let depth=1,inQuote=false,j=i+opener[0].length;for(;j<source.length;j++){const ch=source[j];if(ch==='"'){if(inQuote&&source[j+1]==='"'){j++;continue}inQuote=!inQuote;continue}if(inQuote)continue;if(ch==='(')depth++;else if(ch===')'){depth--;if(depth===0){j++;break}}}if(depth!==0)continue;if(i>cursor)segments.push({type:'text',value:source.slice(cursor,i)});segments.push({type:'formula',value:source.slice(i,j)});cursor=j;i=j-1}if(cursor<source.length)segments.push({type:'text',value:source.slice(cursor)});return segments.length?segments:[{type:'text',value:source}]}
...
function appendReadableNarrative(container,value,wrapperClass='outcome-narrative',label='Agent explanation'){const wrapper=node('div',null,wrapperClass);wrapper.append(node('span',label,'narrative-label'));let proseIndex=0;for(const segment of splitFormulaSegments(value)){if(segment.type==='formula'){const formula=node('div',null,'narrative-formula');formula.append(node('span','Formula referenced','formula-label'),node('code',segment.value));wrapper.append(formula);continue}const readable=String(segment.value||'').replace(/`\s*$/,'').replace(/^\s*`/,'');for(const paragraph of proseParagraphs(readable)){if(/^[\s.,;:!?]+$/.test(paragraph))continue;const copy=node('p',paragraph,proseIndex===0?'narrative-lead':'narrative-copy');wrapper.append(copy);proseIndex++}}container.append(wrapper)}
function appendFalsifierNarrative(container,verdict,experiments=[]){const full=String(verdict?.explanation||'').trim();if(!full)return;const marker=full.search(/\b(?:Experiments executed|All \d+ falsifier test cases pass)\b/i);let summary=full;if(marker>0)summary=full.slice(0,marker).replace(/[\s:;,.]+$/,'');else if(full.length>900)summary=proseParagraphs(full)[0]||full.slice(0,600);appendReadableNarrative(container,summary,'verdict-narrative','Independent check summary');const falsifierTests=(Array.isArray(experiments)?experiments:[]).filter(item=>String(item?.actor||'').toLowerCase().includes('falsifier')).length;const facts=node('div',null,'verdict-facts');for(const [label,value] of [['Independent tests',falsifierTests||'Recorded'],['Counterexamples',verdict.counterexamples?.length||0],['Remaining risks',verdict.remaining_risks?.length||0]]){const fact=node('div',null,'verdict-fact');fact.append(node('span',label),node('b',String(value)));facts.append(fact)}container.append(facts);if(summary!==full){const details=node('details',null,'verdict-transcript');details.append(node('summary','View complete falsifier explanation'));const copy=node('div',full,'verdict-transcript-copy');details.append(copy);container.append(details)}}
...
function render(data){current=data;const result=data.result;const state=data.state||{},decision=state.decision||{};$('results').classList.remove('hidden');$('sourceHash').textContent=result.source_sha256;$('proposalHash').textContent=data.proposal_hash;setSteps(5);$('progressBar').style.width='100%';$('progressBar').parentElement.setAttribute('aria-valuenow','100');$('statusDot').classList.remove('live');$('progressMeta').textContent=`COMPLETE · ${result.decision} · ${result.run_id}`;
 ...
 $('falsifier').replaceChildren();if(v){$('falsifier').append(node('span',v.status,'status '+v.status));appendFalsifierNarrative($('falsifier'),v,data.experiments||[]);if(v.counterexamples?.length)$('falsifier').append(node('div',`Counterexamples: ${v.counterexamples.join('; ')}`,'danger verdict-risk'));if(v.remaining_risks?.length)$('falsifier').append(node('div',`Remaining risks: ${v.remaining_risks.join('; ')}`,'small verdict-risk'))}else $('falsifier').append(node('p','No independent falsifier run was required or completed.'));
