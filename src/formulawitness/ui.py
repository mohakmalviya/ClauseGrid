...
.verify-panel{margin-bottom:18px;display:grid;grid-template-columns:.9fr 1.1fr;gap:22px;align-items:start}.verify-copy h2{font-size:26px;margin:5px 0 10px}.verify-copy p{margin:0}.verify-controls{padding:19px;border:1px solid #cfe4df;border-radius:14px;background:#f6fffc}.verify-mode{display:flex;justify-content:space-between;gap:10px;margin-bottom:15px;color:var(--ok);font-size:11px;font-weight:850}.verify-result{margin-top:15px;padding:15px;border:1px solid var(--line);border-radius:11px;background:#fff}.verify-summary{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.verify-details{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px}.verify-details div{padding:10px;border-radius:8px;background:#f2f6fb;color:#526078;font-size:10px}.verify-details b{display:block;margin-top:4px;color:var(--navy);font-size:13px}.verify-failures{margin:12px 0 0;padding-left:18px;color:var(--bad);font-size:11px;line-height:1.5}
...
button:focus-visible,select:focus-visible,input:focus-visible,a:focus-visible{outline:3px solid rgba(40,103,232,.35);outline-offset:3px}
...
<section id="results" class="hidden" tabindex="-1"><div class="grid results-grid"><div class="panel citation-panel"><h2>Mechanically registered policy citations</h2><div id="citations"></div></div><div class="panel decision-panel"><h2>Agent decision and exact proposal</h2><div id="diagnosis"></div></div></div>
...
function renderVerification(data){const v=data.verification,box=$('verificationResult');box.replaceChildren();const summary=node('div',null,'verify-summary');summary.append(node('span',v.decision,'status '+v.decision),node('b',`${v.passed_count}/${v.test_count} approved tests passed`),node('span',`${v.model_calls} model calls`,'small'));box.append(summary);const details=node('div',null,'verify-details');for(const [label,value] of [['Policy Pack',v.policy_pack_hash.slice(0,12)+'…'],['Test suite',v.test_suite_hash.slice(0,12)+'…'],['Evidence',v.evidence_hash.slice(0,12)+'…']]){const item=node('div');item.append(node('span',label),node('b',value));details.append(item)}box.append(details);if(v.failed_count){const list=node('ul',null,'verify-failures');for(const record of v.records.filter(item=>item.status==='FAIL').slice(0,8)){list.append(node('li',`${record.case_id} · mismatched ${record.mismatched_cells.join(', ')}`))}box.append(list);box.append(node('p',`Affected approved rules: ${v.affected_rule_ids.join(', ')||'none'}`,'small'));if(!v.complete)box.append(node('p',`A policy violation was observed, but coverage was incomplete: ${v.error_count} tests could not execute and ${v.skipped_count} were skipped.`,'small danger'))}else if(v.decision==='INCONCLUSIVE'){box.append(node('p',`Verification was incomplete: ${v.error_count} tests could not execute and ${v.skipped_count} were skipped. No policy violation was observed, so no overall pass or fail is claimed.`,'small danger'))}else box.append(node('p','This workbook matched every generated boundary and retained regression case.','small'));box.classList.remove('hidden');box.focus()}
