...
@media(max-width:900px){.product-intro,.difference-head,.comparison-grid,.hero,.grid,.results-grid{grid-template-columns:1fr}.guide-grid{grid-template-columns:1fr 1fr}.hero-proof{display:none}.tag{display:none}.product-intro{padding:30px}h1{letter-spacing:-1.5px}.evidence-head{display:block}.benchmark-badge{text-align:left;max-width:none}.decision-panel{position:static}.citation-panel #citations{max-height:none;overflow:visible}}
...
<section class="panel difference" aria-labelledby="differenceTitle"><div class="difference-head"><div><span class="section-kicker">Why not just use Claude?</span><h2 id="differenceTitle">Claude can propose. ClauseGrid requires proof.</h2></div><p class="difference-lead"><b>Yes—Claude can read a spreadsheet and policy, explain a suspicious formula, and suggest a correction.</b> ClauseGrid can use Claude or another model as the investigator. Its purpose is to prevent an unverified AI suggestion from becoming a financial control change.</p></div><div class="comparison-grid"><article class="comparison-card"><span class="comparison-label">Claude or another AI assistant</span><h3>Generates a recommendation</h3><ul><li>Analyzes the workbook and policy</li><li>Explains likely problems</li><li>Suggests a corrected formula</li><li>Leaves verification and approval to the user</li></ul></article><article class="comparison-card assurance"><span class="comparison-label">ClauseGrid assurance system</span><h3>Enforces a reviewable decision</h3><ul><li>Binds claims to exact policy citations</li><li>Executes proposed formulas against reproducible counterexamples</li><li>Uses a fresh-context falsifier to attack the proposal</li><li>Preserves the source and blocks writes until human approval</li></ul></article></div><div class="difference-verdict">The difference is enforcement: an AI assistant proposes a fix; ClauseGrid determines whether the evidence is strong enough for that fix to move forward.</div></section>
<section class="guide-grid" aria-label="Product overview"><div class="guide-card"><span class="guide-index">01 / PROBLEM</span><h3>The problem</h3><p>A one-cell formula error can misapply rebates, controls, eligibility rules, or exceptions across thousands of decisions.</p></div><div class="guide-card"><span class="guide-index">02 / NEED</span><h3>Why it is needed</h3><p>High-stakes fixes need citations, executable tests, independent challenge, and approval—not a plausible AI answer.</p></div><div class="guide-card"><span class="guide-index">03 / USERS</span><h3>Who it is for</h3><p>Finance, procurement, operations, compliance, risk, and audit teams using policy-driven workbooks.</p></div><div class="guide-card"><span class="guide-index">04 / TRY IT</span><h3>How to try it</h3><p>Run a controlled benchmark or upload an approved workbook with its governing policy PDF.</p></div></section>
