...
Frozen benchmark: **SupplierRebate-SLA-16** — 12 one-fault mutants, three clean controls, one three-fault hard case, and 48 sealed vectors per workbook.

| System | E2E Semantic Repair Rate | Clean Preservation | Hard case |
|---|---:|---:|---:|
| Direct-agent baseline | 50% (6/12) | 100% | 0% |
| FormulaWitness | **100% (12/12)** | **100%** | **100%** |
| Improvement | **+50 percentage points** | no regression | +100 pp |
...
Open `http://127.0.0.1:8765`, run the M10 witness audit, inspect the page-cited waiver rule and exact `P6` diff, enter a reviewer identity, and approve. The UI then exposes the repaired workbook, `Counterexamples` sheet, `rules.yaml`, diff, evidence graph, report, approval record, and JSONL trajectory.
...
2. `counterexample-agent` generates threshold, precedence, waiver, tenure, cap, and rounding witnesses.
...
The baseline receives the same workbook, policy, formula access, patch scope, and execution budget, but performs one direct audit without structured extraction, generated witnesses, localization, or a critic/replay loop.
...
- The evaluator supports arithmetic, comparisons, direct cell references, and `IF`, `AND`, `OR`, `MAX`, `MIN`, and `ROUND`. Anything else fails closed.
- Approval is immutable and hash-bound. A stale or unexpected old formula cannot be patched.
- Public rule execution labels visible cases; the hidden oracle is independently implemented and never imported by repair workflows.
